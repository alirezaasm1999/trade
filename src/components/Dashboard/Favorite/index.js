  ...
  sortListBaseOnFavorites = async () => {
    try {
      ...
      if (
        ...
        let sortedArray = this.state.results.filter((value, index) => {
          return fovoritesIndexes.includes(value.id);
        });
        await this.setState({
          results: sortedArray,
        ...
        this.forceUpdate();
        console.log("results in Favorite", sortedArray);
      }
    } catch (e) {
      console.log(e);
    }
...
export default withCookies(Favorites);
